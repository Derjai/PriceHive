...
async function getOlimpicaValues(values, titles, links, imgs) {
  ...
  var linksSrc = await Promise.all(
    links.map(async (element) => {
      return await element.evaluate((node) => "https://www.olimpica.com"+node.getAttribute("href"));
    })
  );
...
async function mercadoLibrePrices(Product) {
  try {
    ...
    await page.waitForLoadState("domcontentloaded");
    await page.waitForSelector(".andes-card.ui-search-result--core");

    var products = await page.$$(".andes-card.ui-search-result--core");
    var values = [];
    var titles = [];
    var imgs = [];
    var links = [];
    ...
    for (const product of products) {
      values.push(await product.$(".andes-money-amount__fraction"));
      titles.push(await product.$(".ui-search-item__title"));
      imgs.push(await product.$(".ui-search-result-image__element"));
      links.push(
        await product.$(
...
async function olimpicaPrices(Product) {
  try {
    ...
    var links = await page.$$("a.vtex-product-summary-2-x-clearLink");
    var imgs = await page.$$(
...
async function falabellaPrices(Product) {
  try {
    ...
    var imgs = await page.$$(
      'section.jsx-2469003054 picture > img[width="240"]'
    );
